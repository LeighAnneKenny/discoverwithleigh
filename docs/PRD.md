...
20. **Brands film strip — inverted colour scheme (enhancement).** *(Leigh's feedback 2026-07-19, via Caveshen.)* Flip the hover model: logos **in colour by default**; on hover the strip still pauses (kept), the hovered logo stays in colour, and **the other logos fade to grey**; mouse away → carousel resumes, everything back to colour.
    - **Approach (pure CSS, no JS):** trigger the grey-out with `:has()` scoped to an actual frame hover — `.row:has(.frame:hover) .frame:not(:hover) { filter: grayscale(1) }` — so hovering the sprocket bands/gaps pauses the strip without greying anything (a bare `.row:hover` trigger would grey all logos with none highlighted). Keyboard parity: mirror with `:focus-visible`/`:focus-within` so a focused frame stays colour and greys the rest; teal focus ring kept.
