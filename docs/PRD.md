...
15. **Zero-cost guardrails.** (Caveshen, 2026-07-11: "keep this entire project zero-cost… block any spend… alert at 90% of free-tier quotas.") Verified against Cloudflare docs 2026-07-11:
    ...
    - **Dashboard actions (Caveshen, 5 minutes):** (a) check Billing → Payment info: if no payment method / not Pay-as-you-go, spend is structurally impossible — Cloudflare degrades instead of charging; (b) set a **Budget Alert** at $1 (Billing → Billable Usage → Set Budget Alert, or Notifications → Add → Budget Alert) — emails when *projected* monthly spend crosses the threshold; earliest tripwire Cloudflare offers. There is no hard spend-cap feature; alerts are the ceiling.
    - **Quota sentinel (build post-cutover, needs Email Sending onboarded):** a cron-triggered check in the existing Worker — Workers requests/day, D1 reads+writes/day, R2 storage+ops/month via the GraphQL API — emailing Caveshen at 90% of any free-tier quota. Framed as *availability* protection too: hitting the Workers cap takes the site down (1027) even though it costs nothing. Cut-off logic only where it doesn't self-harm (e.g. pause admin media uploads near the R2 storage ceiling — never degrade the public site to save cents that can't be charged).
