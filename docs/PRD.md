...
15. **Zero-cost guardrails.** (Caveshen, 2026-07-11: "keep this entire project zero-cost… block any spend… alert at 90% of free-tier quotas.") Verified against Cloudflare docs 2026-07-11:
    - **Structurally free (cannot bill on the Free plan — they hard-stop instead):** Workers (100k req/day → Error 1027 / fail-open), D1 (limits → errors), KV (ops fail), Turnstile, Access (≤50 users), Email Sending to verified destinations, Web Analytics. No overage billing exists for these on Free.
    - **The one true spend vector: R2** — usage-billed past 10GB storage / 1M Class A / 10M Class B ops per month *if* the account can bill. Our usage is a rounding error, but it's the vector to guard.
    - **Not cutover-gated — doable any time (established 2026-07-11).** A tidiness/neatness effort in service of the standing zero-cost requirement; nothing here waits on launch.
    - **Dashboard clicks (Caveshen, ~15 minutes, whenever):** (a) check Billing → Payment info: if no payment method / not Pay-as-you-go, spend is structurally impossible — Cloudflare degrades instead of charging; (b) ✅ *2026-07-11* — **Budget Alert set at $0.10** (emails Caveshen when *projected* monthly spend crosses it; earliest tripwire Cloudflare offers — no hard spend-cap feature exists); (c) ~~Email Sending domain onboarding~~ **NOT NEEDED — and not possible on Free** (discovered 2026-07-12: Onboard Domain is Workers Paid only; it gates sending to *arbitrary* recipients). Docs-verified: *"Sending to verified destination addresses in your account is free on all plans, including when only Email Routing is configured"* — our Worker's only recipient is the verified destination, and Email Routing is **already enabled** on the zone (MX `route*.mx.cloudflare.net` + Cloudflare SPF confirmed live), making `discoverwithleigh.co.za` a valid `from:` routing domain. Remaining verification: one manual test enquiry on preview → inbox; (d) create a read-scoped Analytics API token for the sentinel (stored as a Worker secret).
    - **Quota sentinel (build immediately after the clicks):** a cron-triggered check in the existing Worker — Workers requests/day, D1 reads+writes/day, R2 storage+ops/month via the GraphQL API — emailing Caveshen at 90% of any free-tier quota. Framed as *availability* protection too: hitting the Workers cap takes the site down (1027) even though it costs nothing. Cut-off logic only where it doesn't self-harm (e.g. pause admin media uploads near the R2 storage ceiling — never degrade the public site to save cents that can't be charged). *Technical verify at build: `scheduled()` handler through the Astro adapter's Worker entry.*
...
1. ~~Email Sending onboarding~~ **RETIRED 2026-07-12** — Onboard Domain is Workers Paid only and unnecessary: verified-destination sends are free on all plans and Email Routing is already enabled on the zone (see item 15c). Instead: confirm a test enquiry from preview lands in the inbox, and add a DMARC record (`v=DMARC1; p=none`) while in the DNS — the domain has none.
