...
3. **TikTok rotation.** Replace the two fixed embeds with a pool of video IDs; on each page load the server picks a random pair to render. The pool lives in D1 (the existing `video.tiktokIds` content list already renders whatever it holds and is admin-editable — extend it into the pool and add the random selection at render time; admin v2 add/remove-list-items covers curating it).
4. **Brands strip revisit.** v2 made it a full-bleed logo wall, breaking section width conventions. Decide: auto-scrolling marquee carousel (full width becomes intentional; pause on hover, respect reduced-motion) or a contained static grid inside standard width. Recommendation: marquee. Decision pending workshop.
5. **Light mode + theme toggle.** The site is dark-only today. Add a light theme by swapping the design-token custom properties (all colours flow from `:root` variables in `global.css`) under a `[data-theme="light"]` root attribute; default follows `prefers-color-scheme`, user choice persisted in localStorage; toggle in the top bar per design convention. Mind the artwork: the header lockup and hero wordmark are white-on-transparent — light mode needs the dark logo variant (Logo-03 style) or equivalent, and image overlays re-checked for contrast.
