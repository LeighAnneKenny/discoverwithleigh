...
15. **Zero-cost guardrails.** (Caveshen, 2026-07-11: "keep this entire project zero-cost… block any spend… alert at 90% of free-tier quotas.") Verified against Cloudflare docs 2026-07-11:
    ...
    - **Dashboard clicks (Caveshen, ~15 minutes, whenever):** (a) check Billing → Payment info: if no payment method / not Pay-as-you-go, spend is structurally impossible — Cloudflare degrades instead of charging; (b) set a **Budget Alert** at $1 (Billing → Billable Usage → Set Budget Alert) — emails when *projected* monthly spend crosses the threshold; earliest tripwire Cloudflare offers (no hard spend-cap feature exists); (c) **Email Sending domain onboarding** (Email Service → Email Sending → Onboard Domain) — only adds DKIM/SPF records, does not touch the web DNS serving the old site, and Leigh's email is on Gmail so nothing conflicts. Bonus: enquiry emails start working on preview (form → D1 → inbox verified *before* launch) and cutover step 1 is pre-done; (d) create a read-scoped Analytics API token for the sentinel (stored as a Worker secret).
...
1. **Email Sending onboarding** — *may already be done: item 15 moved this earlier (verify rather than perform).* Onboard `discoverwithleigh.co.za` (dashboard: Email Service → Email Sending → Onboard Domain); check DMARC while there.
