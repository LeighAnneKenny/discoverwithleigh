...
16. **Brands section v3 — film-strip counter-marquee.** *(Leigh's review feedback 2026-07-19: the v2 marquee "looks a bit too similar to other sites" — the grayscale edge-faded logo marquee is the most templated pattern on the web. Workshop outcome: Caveshen's dual counter-scrolling rows + film-strip styling to make the motion unmistakably a photographer's.)*
    ...
    - *Status 2026-07-19:* built (pure-CSS "film gate" — sprocket bands fixed, logos move through them), reviewer approved 7/7, 34 API + 78 UI green, look approved by Caveshen; deployed to preview → awaiting Leigh's verdict. *Verdict 2026-07-19: she likes the strip — refinement requested → item 20.*

20. ✅ *accepted by Leigh 2026-07-20.* **Brands film strip — inverted colour scheme (enhancement).** *(Leigh's feedback 2026-07-19, via Caveshen.)* Flip the hover model: logos **in colour by default**; on hover the strip still pauses (kept), the hovered logo stays in colour, and **the other logos fade to grey**; mouse away → carousel resumes, everything back to colour.
    - **Approach (pure CSS, no JS):** trigger the grey-out with `:has()` scoped to an actual frame hover — `.film-marquee:has(.brand-tile:hover) .brand-tile:not(:hover) img { filter: grayscale(1) }` — so hovering the sprocket bands/gaps pauses the strip without greying anything (a bare container `:hover` trigger would grey all logos with none highlighted). *Scope corrected during build: **marquee-wide, not per-row**. The pause rule (`.film-marquee:hover .strip-track`) freezes both rows, so a per-row grey-out would freeze the bottom row while leaving it in full colour — visibly inconsistent. Both rows dim together.*
    - ~~Keyboard parity: mirror with `:focus-visible`~~ **dropped during build, deliberately.** The tiles are decorative `<figure>`s (`alt=""`, no link), and most sit inside `aria-hidden` marquee duplicates — 56 tiles render, only 14 are non-hidden. Making them focusable to satisfy the mirror would add ~56 tab stops to non-interactive content *and* trip axe's `aria-hidden-focus` rule, failing the a11y test the suite already runs. `:focus-visible` selectors on a non-focusable element never match, so mirroring without `tabindex` is dead code. Revisit only if brand tiles ever become links. Existing teal hover ring unchanged.
    - **Side effects to accept:** touch devices (no hover) now show full colour always; reduced-motion static strip is colour with the same hover grey-out. Existing grayscale-hover UI test assertions updated to the inverted scheme.
    - **Honest counterpoint on record:** grayscale-by-default is the convention because a wall of full-colour brand marks can clash with the site palette — but these sit framed on an ink-dark film base, which should carry it, and it's Leigh's wall. Her call stands.
    - **Done when:** both suites green; screenshots (desktop + mobile, both themes, hover state) reviewed by Caveshen; deployed to preview for Leigh.
    - *Status 2026-07-20: built.* Pure CSS as specced, ~15 changed lines in `Brands.astro`; net-new UI test (item 16 shipped with no brands test despite its status note claiming coverage — worth remembering the PRD can flatter itself). Reviewer approve-with-nits, all four dispositioned: keyboard-parity mirror dropped with reasoning above (its proposed `tabindex` fix would have failed the axe scan); its doubt about the test's `emulateMedia({reducedMotion})` call disproved by deleting the line — `scrollIntoViewIfNeeded` then times out at 30s chasing the moving marquee, so the file-wide `test.use({reducedMotion})` demonstrably doesn't stop this animation; fixed sleeps swapped for `expect.poll`; this PRD text corrected. 37 API + 91 UI green, axe included. Screenshots approved by Caveshen → deployed to preview for Leigh's verdict.
...
19. ✅ *closed 2026-07-20 — no change, current layout retained.* **Client reviews section — presentation ideas board.** *(Leigh's feedback 2026-07-19.)* **Outcome:** workshopped three film-world directions (A: current quote wall / B: instant-film Polaroid prints / C: interview lower-thirds) as a rendered visual pitch with the real testimonials; Caveshen reviewed and judged none an improvement on the current wall. Retained as-is by decision. The "can we do better?" question was asked and answered — no. Design directions preserved below should the question ever reopen. She asked whether reviews could be a one-at-a-time scroller like the TikTok embed (item 3's scroll-snap single-slide pattern — reusable). Honest counterpoint recorded up front: reviews are text — a one-at-a-time carousel hides N−1 testimonials and demands interaction to read social proof, which is why review walls usually show several at once. Workshop options before building: (a) TikTok-style single-slide snap carousel, (b) current layout kept, (c) middle path — e.g. featured quote large with the rest as a compact strip, or auto-advancing quote with reduced-motion fallback. Decision belongs to Leigh with the trade-off stated. *Framing set by Caveshen 2026-07-19: the current layout is acceptable as-is — this workshop is a "can we do this better?" question, not a fix; no urgency, no obligation to change anything. **PARKED** until Caveshen resumes it.*

21. **Travel-visitor targeting — SEO + machine surfaces.** *(Leigh's direction 2026-08-08, via Caveshen: target tourists visiting Cape Town — Airbnb stays, photographer-for-a-day, special occasions — alongside weddings.)* Principle on record: Google and LLM crawlers alike rank **visible copy**; tags and schema reinforce it, never substitute for it. No new pages.
    - **(a) JSON-LD enrichment (GO):** extend the existing homepage ProfessionalService block — `areaServed` (Cape Town), a service list (weddings, vacation/travel shoots, special occasions, content work), keeping `sameAs` derivation from socials (item 7). Derive from `loadContent()` where values exist so admin edits flow through; hardcode only stable identity facts. Head-only script — not user-visible. No Review/aggregateRating on own testimonials (cutover step 9's DON'T).
    - **(b) Meta/OG + llms.txt (GO):** meta description/OG copy retuned toward travel queries; the dynamic llms.txt gains an audience/services line saying the same in prose.
    - **(c) Copy amendments (PENDING Caveshen):** proposals to be tabled from a copy survey. Pre-live, so seed/code edits are as acceptable as admin edits.
    - **(d) Gallery category for vacation shoots (PENDING):** mechanics report requested; needs photos from Leigh regardless.
    - **Parked:** a dedicated `/vacation-photography` landing page — Caveshen: "absolutely not" for now; revisit only if the cheap layers underperform post-launch and Leigh wants it.
    - **Done when:** schema validates, both suites green, rendered head + llms.txt output reviewed by Caveshen before commit/deploy; no PII in tracked source (identity facts already published on the rendered site are fine).
    - *Status 2026-08-08: (a)+(b) built.* `knowsAbout` extended (photography categories from content + two hardcoded travel strings), meta/OG description retuned (152 chars), llms.txt audience line + guard test. 38 API + 91 UI green. Reviewer approve-with-nits, all three dispositioned accept-as-is: hardcoded travel strings stay until 21(d) gives them a content source to derive from; `og:image:alt` deliberately NOT keyword-retuned (alt text describes the image — the lockup — not search queries; `og:description` already carries the retuned copy); three-file literal spread accepted (three short strings in three registers; a shared source would over-engineer). Awaiting Caveshen's review gate before commit/deploy.

22. **Leigh's off-site launch guide (docs).** *(Caveshen 2026-08-08: collect all non-website growth steps into a repo guide so they permanently leave the dev board and land in Leigh's hands.)* `docs/leigh-guide.md`, written for Leigh — plain language, step-by-step, no dev jargon, no PII: Google Business Profile setup (categories, service area, photos, the client-review habit — mirrors cutover step 9), WhatsApp Business greeting/away (absorbs item 5's links, draft copy, and decision list), and profile-consistency notes (same name/number/area everywhere). On landing, item 5 retires from WAITING ON LEIGH with a pointer to the guide.

# WAITING ON LEIGH

- **Site review** — the current pause; feedback may spawn one more work round.
- ~~Item 14 verdict~~ ✅ **accepted by Leigh 2026-07-20** — aperture shutter-click v4 signed off after three feedback rounds (recorded in SHIPPED). Closed.
- ~~Item 16 verdict~~ ✅ arrived 2026-07-19 — she likes the film strip; her refinement (colour-first hover inversion) is item 20.
- ~~Item 20 verdict~~ ✅ **accepted by Leigh 2026-07-20** — film-strip colour inversion signed off. Closed.
...
- ~~Item 5 — WhatsApp auto-response~~ **retired to `docs/leigh-guide.md` 2026-08-08 (item 22)** — the guide absorbs the FAQ links, draft copy, and decision list below; permanently off the dev board, in Leigh's hands. Original detail kept for the record:
