...
4. **Brands section v2** *(expanded 2026-07-06; see also Bug fixes #2).* Two-parter:
   - **(a) Bug fix — contain the strip.** Bring the logo wall inside standard section width on desktop. Presentation options: auto-scrolling marquee (single row, infinite loop, pause on hover, reduced-motion renders static — industry standard for "brands we've worked with" and the recommendation), scroll-snap carousel matching the gallery, or a capped static grid. Decision pending workshop.
   - **(b) Enhancement — admin-customisable tiles.** Each tile becomes logo + optional text label, with a per-tile **on/off toggle** so Leigh can hide brands without deleting them. Requires: `brands` table gains `label TEXT` and `enabled INTEGER` columns (migration), render honours both, and the admin Brands tab grows label editing + a visibility toggle alongside the existing upload/reorder/delete.
