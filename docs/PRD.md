...
17. **Gallery: selected image should open centered.** *(Leigh's feedback 2026-07-19.)* Selecting an image to view in the gallery carousel doesn't present it centered. Investigate what "select to view" currently does in the scroll-snap carousel and make the chosen image open/scroll centered in the viewport (likely `scroll-snap-align: center` / `scrollIntoView({inline:'center'})` territory — confirm at build time). Enhancement, not a launch blocker.
...
18. **Admin→site round-trip test coverage.** *(Prompted by bug #7 — Leigh edited a title in admin and the site didn't change, while other edits worked.)* Close the class of "admin exposes a field the site never renders" (and its inverse) mechanically, in the pipeline, not on live:
    ...
    - Bug #7's fix (drop or wire the dead field) lands with this item.
